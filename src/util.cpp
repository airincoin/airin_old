...
const char * const BITCOIN_CONF_FILENAME = "airin.conf";
const char * const BITCOIN_PID_FILENAME = "airind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "airin" is a composite category enabling all Airin-related debug output
            if(ptrCategory->count(string("airin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AirinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AirinCore
    // Mac: ~/Library/Application Support/AirinCore
    // Unix: ~/.airincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AirinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AirinCore";
#else
    // Unix
    return pathRet / ".airincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty airin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override airin.conf
